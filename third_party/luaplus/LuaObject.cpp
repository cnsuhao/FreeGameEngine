...
/* GetUserType: add by zhoubao, Mirrors tolua++: tolua_tousertype().*/
LUAPLUS_CLASS_API void* LuaObject::GetUserType()
{
    void **p = (void**) this->GetUserData();
    return *p;
}
...
inline void LuaObject::AddToUsedList(lua_State* state)
{
...
inline void LuaObject::AddToUsedList(lua_State* state, const lua_TValue& obj)
{
	luaplus_assert(state);
    lua_lock(state);
	L = state;
