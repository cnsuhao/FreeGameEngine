...
\#ifndef TOLUA_RELEASE
    if (!self)
    {
        tolua_error(tolua_S,"invalid 'self' in function '${signature_name}'", nullptr);
        return 0;
    }
\#endif
#end if
...
#for func in $implementations
    ...
    do{
        ...
        if (argc == ${arg_idx}) {
            ...
            int ret = (int)self->${func.func_name}($arg_list);
                #else
            ${func.ret_type.get_whole_name($generator)} ret = self->${func.func_name}($arg_list);
                #end if
            ${func.ret_type.from_native({
                "generator": $generator,
                "in_value": "ret",
                "out_value": "ret",
...
#end if
#end for
    luaL_error(tolua_S, "%s has wrong number of arguments: %d, was expecting %d \n", lua_fun_name, argc, ${func.min_args});
    return 0;
